...
// Email login endpoint for frontend compatibility

// SSO Login - authenticates users via GHL userId and locationId
app.post('/api/auth/sso-login', loginRateLimit, async (req, res) => {
  const { locationId, userId } = req.body;

  if (!locationId || !userId) {
    return res.status(400).json({ success: false, error: 'locationId and userId required' });
  }

  // Verify location exists
  const locationConfig = ALLOWED_LOCATIONS[locationId];
  if (!locationConfig) {
    return res.status(401).json({ success: false, error: 'Location not authorized' });
  }

  // Find user by ID in the location's users
  const staffMember = (locationConfig.users || locationConfig.staff)?.find(
    s => s.id === userId || s.odooUid === userId || s.odoo_uid === userId
  );

  if (!staffMember) {
    return res.status(401).json({ success: false, error: 'User not found in location' });
  }

  const token = jwt.sign(
    {
      odooUid: staffMember.id || staffMember.odooUid || staffMember.odoo_uid,
      odooDb: 'leaddash',
  ...
  res.json({
    ...
    token,
    user: {
      userId: staffMember.id || staffMember.odooUid || staffMember.odoo_uid,
      userName: staffMember.name || (staffMember.firstName + ' ' + staffMember.lastName),
      userEmail: staffMember.email,
      userType: staffMember.role || 'account-user',
      locationId: locationId,
...
// DISABLED for security - use SSO only
app.post('/api/auth/email-login-DISABLED', loginRateLimit, async (req, res) => {
  const { email } = req.body;
  ...
  const locationId = Object.keys(ALLOWED_LOCATIONS)[0];
  const locationConfig = ALLOWED_LOCATIONS[locationId];
  
  if (!locationConfig) {
    return res.status(401).json({ success: false, error: 'No locations configured' });
  }
  
  const staffMember = (locationConfig.users || locationConfig.staff)?.find(s => s.email?.toLowerCase() === email.toLowerCase());
  ...
  const token = jwt.sign(
    {
      odooUuserId: staffMember.id || staffMember.odooUid || staffMember.odoo_uid,
      odooDb: 'leaddash',
      locationId: locationId,
      userEmail: staffMember.email,
      userName: staffMember.name || (staffMember.firstName + ' ' + staffMember.lastName),
      userType: staffMember.role || 'account-user',
      isAdmin: staffMember.role === 'agency-admin' || staffMember.role === 'account-admin'
    },
    EFFECTIVE_JWT_SECRET,
    { expiresIn: '8h' }
  );
